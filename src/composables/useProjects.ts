...
export function useProjects(textareaRef: any, newMessage: any) {
  ...
  const checkForProjectTag = () => {
    ...
    if (hashIndex !== -1 && !showProjectPopover.value) {
      ...
      const isValidHashPosition = charBeforeHash === "" || /\s/.test(charBeforeHash);
      ...
      if (isValidHashPosition) {
        ...
        console.log("Popover shown", { position: projectPopoverPosition.value });
      }
  ...
  const selectProject = async (project: ProjectRecord) => {
    ...
    if (existingProjectMatch) {
      ...
      newMessage.value = message.substring(0, message.length - existingProjectMatch[0].length).trim() + 
                          " in #" + project.name;
    } else {
      ...
      if (hashIndex !== -1) {
        ...
        const endOfHashWord = spaceIndex !== -1 ? hashIndex + spaceIndex : message.length;

        // Get the cleaned message without the partial project tag
        const cleanedMessage = message.substring(0, hashIndex) + 
