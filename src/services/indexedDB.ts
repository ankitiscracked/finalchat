...
export async function deleteItems(itemIds: number[]): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    itemIds.forEach((itemId) => {
      ...
      request.onerror = () => {
        console.error(
          "Error deleting item from IndexedDB, ID:",
          itemId,
          request.error
        );
        errorOccurred = true;
    ...
    transaction.oncomplete = () => {
      if (errorOccurred) {
        reject(new Error("One or more items failed to delete."));
      } else {
    ...
    transaction.onerror = () => {
      console.error(
        "Transaction error while deleting items:",
        transaction.error
      );
...
export async function addProject(
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      console.log(
        "Project added successfully to IndexedDB, ID:",
        request.result
      );
      resolve(request.result as number);
    };
...
export async function getAllProjects(): Promise<ProjectRecord[]> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      console.log(
        `Retrieved ${request.result.length} projects from IndexedDB.`
      );
...
export async function addCollection(
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      console.log(
        "Collection added successfully to IndexedDB, ID:",
        request.result
      );
      resolve(request.result as number);
    };
...
export async function getAllCollections(): Promise<CollectionRecord[]> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      console.log(
        `Retrieved ${request.result.length} collections from IndexedDB.`
      );
      // Ensure dates are Date objects
